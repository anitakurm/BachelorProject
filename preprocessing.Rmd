...
d$day<-as.factor(format(d$Datetime, "%d"))
d$hour<-as.factor(format(d$Datetime, "%d %H"))
d$minute<-as.factor(format(d$Datetime, "%d %H:%M"))
...
statsday<-d %>%
  group_by(day) %>%
  summarise(meanD=mean(Compound), sdD=sd(Compound), minD=min(Compound),maxD=max(Compound), varD=var(Compound), rateD=n(), posD=mean(Positive), negD=mean(Negative), neutD=mean(Neutral))

statshour<-d %>%
  group_by(hour) %>%
  summarise(meanH=mean(Compound), sdH=sd(Compound), minH=min(Compound),maxH=max(Compound), varH=var(Compound), rateH=n(), posH=mean(Positive), negH=mean(Negative), neutH=mean(Neutral))

statsminute<-d %>%
  group_by(minute) %>%
  summarise(meanM=mean(Compound), sdM=sd(Compound), minM=min(Compound),maxM=max(Compound), varM=var(Compound), rateM=n(), posM=mean(Positive), negM=mean(Negative), neutM=mean(Neutral))
...
write.csv(ultimate,"processeddata.csv")
```
